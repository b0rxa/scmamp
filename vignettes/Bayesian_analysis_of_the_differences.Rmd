...
From the results we can see that, with high confidence, we can conclude that Algorithms C and D are practically equivalent according to the 10 datasets in our data. Conversely, if we compare algorithms A and B, the conclusion is not that clear.
...
In the plot we can clearly see that, although there is quite uncertainty about A being better or equal to B, we can be very certain that it is not worse. Moreover, the samples are concentrated in the Algorithm A-Rope line, meaning that the probability of being bellow the rope is virtually zero. We can assess this with the expected probabilities as calculated in the previous section:
